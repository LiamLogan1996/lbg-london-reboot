...
def inflation(request):
    ...
    context_dict['sub_percent'] = percent(context_dict['sub_spend'], salary)
    context_dict['food_percent'] = percent(context_dict['food_spend'], salary)
    context_dict['restaurant_percent'] = percent(context_dict['restaurant_spend'], salary)

    for prod in allProducts:
        if prod.ProductCategory == "Bills":            
            bill_itemInfo[prod.ProductName] = [prod.ProductPrice, round(prod.ProductPrice * 1.117, 2), round(prod.ProductPrice * 1.117, 2) - prod.ProductPrice] #11/7% increase
            bill_total_inflation += round(prod.ProductPrice * 1.117, 2) - prod.ProductPrice
            bill_table = [[key, values[0], values[1], round(values[2], 2)] for key, values in bill_itemInfo.items()]

        elif prod.ProductCategory == "Food":
            food_itemInfo[prod.ProductName] = [prod.ProductPrice, round(prod.ProductPrice * 1.164, 2), round(prod.ProductPrice * 1.164, 2) - prod.ProductPrice] #16.4% increase
            food_total_inflation += round(prod.ProductPrice * 1.164, 2) - prod.ProductPrice
            food_table = [[key, values[0], values[1], round(values[2], 2)] for key, values in food_itemInfo.items()]
    
        elif prod.ProductCategory == "Restaurants":
            restaurant_itemInfo[prod.ProductName] = [prod.ProductPrice, round(prod.ProductPrice * 1.087, 2), round(prod.ProductPrice * 1.096, 2) - prod.ProductPrice] #9.6% increase
            restaurant_total_inflation +=  round(prod.ProductPrice * 1.096, 2) - prod.ProductPrice
            restaurant_table = [[key, values[0], values[1], round(values[2], 2)] for key, values in restaurant_itemInfo.items()]

        elif prod.ProductCategory == "Alcohol":
            alcohol_itemInfo[prod.ProductName] = [prod.ProductPrice, round(prod.ProductPrice * 1.062, 2), round(prod.ProductPrice * 1.062, 2) - prod.ProductPrice] #6.2% increase
            alcohol_total_inflation += round(prod.ProductPrice * 1.062, 2) - prod.ProductPrice
            alcohol_table = [[key, values[0], values[1], round(values[2], 2)] for key, values in alcohol_itemInfo.items()]

        elif prod.ProductCategory == "Subscriptions":
            subscriptions_itemInfo[prod.ProductName] = [prod.ProductPrice, round(prod.ProductPrice * 1.051, 2),round(prod.ProductPrice * 1.051, 2) -  prod.ProductPrice] #5.1% increase
            sub_total_inflation += round(prod.ProductPrice * 1.051, 2) -  prod.ProductPrice
            sub_table = [[key, values[0], values[1], round(values[2], 2)] for key, values in subscriptions_itemInfo.items()]
